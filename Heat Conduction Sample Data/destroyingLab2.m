...
alpha_AL = zeros(1,4);
slegged = linspace(0.2,0.5,4)
% calculating thermal diffusivity contant
for sleg = 1:4
    alpha_AL(sleg) = k_AL / (cp_AL * density_AL) * slegged(sleg);
    alpha_BR(sleg) = k_BR / (cp_BR * density_BR) * slegged(sleg);
    alpha_ST(sleg) = k_ST / (cp_ST * density_ST) * slegged(sleg);
    ...
    %             sum_AL_25V_exp = sum_AL_25V_exp + -(8 .* H_any(1,1) .* L .* (-1).^n)./(pi.^2 .* (2 .* n - 1).^2) .* sin(j .* ((2 .* n - 1) .* pi)./(2 .* L)) .* exp(-(((2 .* n - 1) .* pi)./(2 .* L)).^2 .* alpha_AL .* i);
                sum_AL_25V_any = sum_AL_25V_any + -(8 .* H_any(1,1) .* L .* (-1).^n)./(pi.^2 .* (2 .* n - 1).^2) .* sin(j .* ((2 .* n - 1) .* pi)./(2 .* L)) .* exp(-(((2 .* n - 1) .* pi)./(2 .* L)).^2 .* alpha_AL(sleg) .* i);
    %             sum_AL_28V_exp = sum_AL_28V_exp + -(8 .* H_any(1,2) .* L .* (-1).^n)./(pi.^2 .* (2 .* n - 1).^2) .* sin(j .* ((2 .* n - 1) .* pi)./(2 .* L)) .* exp(-(((2 .* n - 1) .* pi)./(2 .* L)).^2 .* alpha_AL .* i);
                sum_AL_28V_any = sum_AL_28V_any + -(8 .* H_any(1,2) .* L .* (-1).^n)./(pi.^2 .* (2 .* n - 1).^2) .* sin(j .* ((2 .* n - 1) .* pi)./(2 .* L)) .* exp(-(((2 .* n - 1) .* pi)./(2 .* L)).^2 .* alpha_AL(sleg) .* i);
    %             sum_BR_26V_exp = sum_BR_26V_exp + -(8 .* H_any(1,3) .* L .* (-1).^n)./(pi.^2 .* (2 .* n - 1).^2) .* sin(j .* ((2 .* n - 1) .* pi)./(2 .* L)) .* exp(-(((2 .* n - 1) .* pi)./(2 .* L)).^2 .* alpha_BR .* i);
                sum_BR_26V_any = sum_BR_26V_any + -(8 .* H_any(1,3) .* L .* (-1).^n)./(pi.^2 .* (2 .* n - 1).^2) .* sin(j .* ((2 .* n - 1) .* pi)./(2 .* L)) .* exp(-(((2 .* n - 1) .* pi)./(2 .* L)).^2 .* alpha_BR(sleg) .* i);
    %             sum_BR_29V_exp = sum_BR_29V_exp + -(8 .* H_any(1,4) .* L .* (-1).^n)./(pi.^2 .* (2 .* n - 1).^2) .* sin(j .* ((2 .* n - 1) .* pi)./(2 .* L)) .* exp(-(((2 .* n - 1) .* pi)./(2 .* L)).^2 .* alpha_BR .* i);
                sum_BR_29V_any = sum_BR_29V_any + -(8 .* H_any(1,4) .* L .* (-1).^n)./(pi.^2 .* (2 .* n - 1).^2) .* sin(j .* ((2 .* n - 1) .* pi)./(2 .* L)) .* exp(-(((2 .* n - 1) .* pi)./(2 .* L)).^2 .* alpha_BR(sleg) .* i);
    %             sum_ST_21V_exp = sum_ST_21V_exp + -(8 .* H_any(1,5) .* L .* (-1).^n)./(pi.^2 .* (2 .* n - 1).^2) .* sin(j .* ((2 .* n - 1) .* pi)./(2 .* L)) .* exp(-(((2 .* n - 1) .* pi)./(2 .* L)).^2 .* alpha_ST .* i);
                ...
                sum_ST_21V_any = sum_ST_21V_any + -(8 .* H_any(1,5) .* L .* (-1).^n)./(pi.^2 .* (2 .* n - 1).^2) .* sin(j .* ((2 .* n - 1) .* pi)./(2 .* L)) .* exp(-(((2 .* n - 1) .* pi)./(2 .* L)).^2 .* alpha_ST(sleg) .* i);
            end
    %         test = sum_ST_21V_exp;
...
%     xlim([0,2600])
%     title("Aluminum at 28V and Alpha = " + alpha_AL(sleg) + " n = " + slegged(sleg))
%     xlabel("Time [s]")
...
%     xlim([0,2600])
%     title("Aluminum at 25V and Alpha = " + alpha_AL(sleg) + " n = " + slegged(sleg))
%     xlabel("Time [s]")
%     ylabel("Temperature [C]")
...
%     xlim([0,5000])
%     title("Brass at 26V and Alpha = " + alpha_BR(sleg) + " n = " + slegged(sleg))
%     xlabel("Time [s]")
%     ylabel("Temperature [C]")
...
%     xlim([0,5000])
%     title("Brass at 29V and Alpha = " + alpha_BR(sleg) + " n = " + slegged(sleg))
%     xlabel("Time [s]")
%     ylabel("Temperature [C]")
...
%     xlim([0,12000])
%     title("Steel at 21V and Alpha = " + alpha_ST(sleg) + " n = " + slegged(sleg))
%     xlabel("Time [s]")
%     ylabel("Temperature [C]")
